...
internal class DomainProviderTest {
    ...
    inner class FindInMethodInvocation {
        ...
        fun `returns Domain annotation from method`() {
            val methodInvocation =
                methodInvocationMethod(
                    ExampleBeanWithDomain::class,
                    "exampleMethodWithDomainAnnotation",
                )

            assertThat(domainProvider.findInAopCall(methodInvocation)).isEqualTo(
        ...
        fun `returns Domain annotation from bean if method has no annotation`() {
            val methodInvocation =
                methodInvocationMethod(
                    ExampleBeanWithDomain::class,
                    "exampleMethodWithoutDomainAnnotation",
                )
        ...
        fun `returns null if neither method nor bean have Domain annotation`() {
            ...
            val methodInvocation =
                methodInvocationMethod(
                    ExampleBeanWithoutDomain::class,
                    "exampleMethodWithoutDomainAnnotation",
                )
        ...
        private fun methodInvocationMethod(
            beanClass: KClass<*>,
            methodName: String,
        ): MethodInvocation {
    ...
    inner class FindInHandlerMethod {
        ...
        fun `returns Domain annotation from method`() {
            ...
            assertThat(domainProvider.findInMvcCall(handlerMethod)).isEqualTo(
                io.github.feddena.monolith.splitter.Domain(
                    "CHAT",
                ),
            )
...
private class ExampleBeanWithDomain {
    @io.github.feddena.monolith.splitter.Domain("CHAT")
